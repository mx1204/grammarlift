...
export default function ScenarioPlayerPage() {
  ...
  // Try finding in both data sources
  const scenario = scenarios.find(s => s.id === id);
  const interpersonalScenario = interpersonalScenarios.find(s => s.id === id);
  
  const [selectedOption, setSelectedOption] = useState<number | null>(null);
  const [isSubmitted, setIsSubmitted] = useState(false);

  if (!scenario && !interpersonalScenario) {
    return (
  ...
  // Handle Interpersonal Mode
  if (interpersonalScenario) {
    return (
          ...
          <div>
            <header style={{ marginBottom: '2rem' }}>
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                <span className="glass" style={{ fontSize: '0.75rem', padding: '4px 12px', borderRadius: '20px' }}>{interpersonalScenario.category}</span>
                <span className="glass" style={{ fontSize: '0.75rem', padding: '4px 12px', borderRadius: '20px' }}>{interpersonalScenario.difficulty}</span>
              </div>
              <h1 style={{ fontSize: '2.5rem', fontWeight: 800 }}>{interpersonalScenario.title}</h1>
            </header>
            ...
            <GlassCard style={{ padding: '2rem', marginBottom: '2rem', borderLeft: '4px solid var(--primary)' }}>
              ...
              <p style={{ fontSize: '1.1rem', lineHeight: 1.6 }}>{interpersonalScenario.context}</p>
            </GlassCard>
            ...
            <div style={{ marginBottom: '3rem' }}>
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                <div style={{ 
                  ...
                  {interpersonalScenario.character.name[0]}
                </div>
                <GlassCard style={{ flex: 1, padding: '1.5rem' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 700, display: 'block', marginBottom: '0.5rem', color: 'var(--primary)' }}>
                    {interpersonalScenario.character.name} ({interpersonalScenario.character.role})
                  </span>
                  <p style={{ fontSize: '1.2rem', fontWeight: 500 }}>&quot;{interpersonalScenario.prompt}&quot;</p>
                </GlassCard>
              </div>
            </div>

            <InterpersonalTrainer scenario={interpersonalScenario} />
          </div>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <GlassCard>
              ...
              <ul style={{ marginTop: '1rem', paddingLeft: '1.2rem', fontSize: '0.9rem', color: 'var(--text-muted)', lineHeight: 1.7 }}>
                {interpersonalScenario.learningObjectives.map((obj, i) => (
                  <li key={i}>{obj}</li>
                ))}
              </ul>
            </GlassCard>
            ...
            <GlassCard>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <h5 style={{ fontSize: '1.1rem' }}>{interpersonalScenario.character.name}</h5>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>{interpersonalScenario.character.description}</p>
              </div>
            </GlassCard>
          </aside>
        </div>
  ...
  // Handle Standard Scenario Mode
  if (!scenario) return null; // Should not happen due to previous check, but satisfies TS
